...
function aiDraftFromText_(text, ui, traceId) {
  ...
  const sys = [
    ...
    "",
    "### 1. merchant (店名) - MUST be SHORT",
    "- Extract ONLY the store/merchant name (2-10 characters)",
    "- '自販機にてコーヒーを150円' → merchant: '自販機'",
    "- 'スタバでラテを買った' → merchant: 'スタバ'",
    "- 'イオンで食料品' → merchant: 'イオン'",
    "- NEVER include '〜にて', '〜で', product names in merchant",
    "",
    "### 2. title - Same as merchant or short description",
    "- Usually same as merchant: '自販機', 'スタバ'",
    "- If no merchant: use item name like 'コーヒー', '電気代'",
    "",
    "### 3. category - MUST choose from allowed_categories list",
    "- Look at allowed_categories in user input",
    "- Pick the BEST matching one",
    "- For food/drinks: '食費'",
    "- If no match: use empty string ''",
    "",
    "### 4. payment_method - MUST choose from allowed_payment_methods list",
    "- Look at allowed_payment_methods in user input",
    "- '楽天カード', 'JCBカード', 'クレジットカード' etc.",
    "- Match user's mention to the list",
    "- If no match: use empty string ''",
    "",
    "### 5. Date (occurred_at)",
    "- Extract the OCCURRENCE DATE, NOT today",
    "- '12月のボーナス' → '" + thisYear + "-12-01'",
    "- No date mentioned → '" + today + "'",
    "- Today is: " + today,
    "",
    "### 6. Transaction Type (txn_type)",
    "- expense: 普通の支出（食費、交通、娯楽など）",
    "- income: 収入（給料、ボーナス、売上）",
    "- fixed_cost: 固定費（家賃、光熱費、通信費、保険、サブスク）",
    "",
    ...
    "- '収入として' → income",
    "",
    "### 7. memo - Short clean summary",
    "- Brief description of the transaction"
  ].join("\n");

  const user = {
    instruction: "Extract transaction. CRITICAL: category MUST be from allowed_categories, payment_method MUST be from allowed_payment_methods. merchant should be SHORT (just store name like '自販機', 'スタバ').",
    input_text: text,
    today,
    this_year: thisYear,
